...
require 'support/password_spec_helper'

RSpec.describe 'User', type: :system do
  include ActionMailer::TestHelper
  include ApplicationSystemSpecHelper
  include PasswordSpecHelper
  ...
  it 'can sign up' do
    ...
    expect(page).to have_text('Registro')

    expect(page).to have_text('Continuar con Google')
    ...
    within('.form-input-container', text: 'Confirma tu nueva contraseña') do
      expect(page).to have_text('No coincide')
    end
  ...
  it 'can login' do
    ...
    expect(page).to have_text('Ingreso')
    expect(page).to have_text('Continuar con Google')
  ...
  it 'can edit profile' do
    ...
    within('.form-input-container', text: 'Contraseña actual') do
      expect(page).to have_text('No es válido')
    end
    ...
    within('.form-input-container', text: 'Nueva contraseña') do
      expect(page).to have_text('Es demasiado corto (6 caracteres mínimo)')
    end
