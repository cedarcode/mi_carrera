class LogicalPrerequisite < Prerequisite
  ...
  def met?(student)
    ...
    when "and"
      operands_prerequisites.all? { |prereq| prereq.met?(student) }
    when "or"
      operands_prerequisites.any? { |prereq| prereq.met?(student) }
    when "not"
      operands_prerequisites.none? { |prereq| prereq.met?(student) }
    when "at_least"
      operands_prerequisites.count { |prereq| prereq.met?(student) } >= amount_of_subjects_needed
    end
  end
