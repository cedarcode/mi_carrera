class Review < ApplicationRecord
  ...
  validate :at_least_one_review_field_present, on: :create

  after_save :destroy_if_all_fields_blank
  ...
  # Check if all review fields are blank
  def all_review_fields_blank?
    self.class.review_fields.all? { |field| send(field).nil? }
  end

  private

  def at_least_one_review_field_present
    if all_review_fields_blank?
      errors.add(:base, "At least one review field must be present")
    end
  end

  def destroy_if_all_fields_blank
    destroy! if all_review_fields_blank?
  end
end
