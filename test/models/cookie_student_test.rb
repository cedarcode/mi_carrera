...
class CookieStudentTest < ActiveSupport::TestCase
  test "#add adds approvable.id only if available" do
    ...
    assert_equal hash_to_cookie([subject1.course.id]), cookies[:approved_approvable_ids]

    student.add(subject2.course)
    assert_equal hash_to_cookie([subject1.course.id, subject2.course.id]), cookies[:approved_approvable_ids]
  end
  ...
  test "#remove removes approvable.id and other approvables that are not available anymore" do
    ...
    cookies = {
      approved_approvable_ids: hash_to_cookie([subject1.course.id, subject2.course.id, subject3.course.id,
                                               subject4.course.id])
    }
    student = CookieStudent.new(cookies)
    ...
    assert_equal hash_to_cookie([subject4.course.id]), cookies[:approved_approvable_ids]
  end
  ...
  test "#available? returns true if subject_or_approvable is available" do
    ...
    assert CookieStudent.new({
                               approved_approvable_ids: hash_to_cookie([subject1.course.id])
  ...
  test "#approved? returns true if subject_or_approvable is approved" do
    ...
    assert CookieStudent.new({ approved_approvable_ids: hash_to_cookie([subject1.course.id]) }).approved?(subject1)
    assert CookieStudent.new({
                               approved_approvable_ids: hash_to_cookie([subject1.course.id])
    ...
    assert_not CookieStudent.new({ approved_approvable_ids: hash_to_cookie([subject2.course.id]) }).approved?(subject2)

    assert CookieStudent.new({ approved_approvable_ids: hash_to_cookie([subject2.exam.id]) }).approved?(subject2)
    assert_not CookieStudent.new({
                                   approved_approvable_ids: hash_to_cookie([subject2.exam.id])
                                 }).approved?(subject2.course)
    assert CookieStudent.new({ approved_approvable_ids: hash_to_cookie([subject2.exam.id]) }).approved?(subject2.exam)
  end
  ...
  test "#group_credits returns approved credits for the given group" do
    ...
    student = CookieStudent.new(approved_approvable_ids: hash_to_cookie([subject1.course.id]))
    assert_equal 10, student.group_credits(group1)
    student = CookieStudent.new(approved_approvable_ids: hash_to_cookie([subject1.course.id, subject2.course.id]))
    assert_equal 10, student.group_credits(group1)
    student = CookieStudent.new(approved_approvable_ids: hash_to_cookie([subject1.course.id, subject2.exam.id]))
  ...
  test "#add subject.exam adds subject.course as well" do
    ...
    assert_equal hash_to_cookie([subject.exam.id, subject.course.id]), cookies[:approved_approvable_ids]
  end
end
